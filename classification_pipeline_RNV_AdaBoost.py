...
base_seed = 42
base_output_dir = 'results_AdaBoost_RNV'
...
def save_summary_metrics(
    fold_results, file_path, seed, clf_name, resample_name, method_name, n_splits
):
...
def save_confusion_matrix_plot(cm_array, title, output_path):
  cm_pct = (cm_array.astype('float') / cm_array.sum(axis=1)[:, np.newaxis]) * 100
...
#Inner Precomputation & Optuna Optimization

def precompute_inner_fold_data(X_outer_train, y_outer_train, resample_name, seed):
...
def create_strict_optuna_objective(
    precomputed_inner_data, search_n_estimators, threshold_search=False
):
  def objective(trial):
...
def build_final_adaboost(best_params_flat, n_estimators):
  base_tree = DecisionTreeClassifier(
      criterion=best_params_flat['criterion'],
      splitter=best_params_flat['splitter'],
      max_depth=best_params_flat['max_depth'],
      min_samples_split=best_params_flat['min_samples_split'],
      min_samples_leaf=best_params_flat['min_samples_leaf'],
      max_features=best_params_flat['max_features'],
      random_state=8,
  )
...
for resample_method in resampling_methods:
  target_dir = os.path.join(
      base_output_dir, resample_method, f'Optimized_{target_metric.capitalize()}'
  )
  dir_m05 = os.path.join(target_dir, 'Method_Default_Threshold_0.5')
  dir_youden = os.path.join(target_dir, 'Threshold_Tuning_YoudenJ')
  ...
  for fold_idx, (train_idx, test_idx) in enumerate(outer_cv.split(X, y)):
    ...
    selected_indices = [i for i, coef in enumerate(lasso_out.coef_[0]) if coef != 0]
    ...
    precomputed_inner_data = precompute_inner_fold_data(
        X_train_outer, y_train_outer, resample_method, current_seed
    )
    ...
    best_ada_05 = build_final_adaboost(best_params_05, FINAL_N_ESTIMATORS)
    best_ada_05.fit(X_tr_res_final, y_tr_res_final)

    y_tr_proba_05 = best_ada_05.predict_proba(X_tr_selected)[:, 1]
    y_te_proba_05 = best_ada_05.predict_proba(X_te_selected)[:, 1]
    ...
    opt_obj_yj = create_strict_optuna_objective(
        precomputed_inner_data, SEARCH_N_ESTIMATORS, threshold_search=True
    )
    ...
    best_ada_yj = build_final_adaboost(best_params_yj, FINAL_N_ESTIMATORS)
    best_ada_yj.fit(X_tr_res_final, y_tr_res_final)

    y_tr_proba_yj = best_ada_yj.predict_proba(X_tr_selected)[:, 1]
    y_te_proba_yj = best_ada_yj.predict_proba(X_te_selected)[:, 1]
  ...
  clf_label = f'AdaBoost_{resample_method}_{target_metric.capitalize()}'
...
master_excel_path = os.path.join(
    base_output_dir, 'AdaBoost_Optuna_Master_Summary.xlsx'
)

...
base_seed = 42
base_output_dir = 'results_DT_RNV'
...
def resampler_label(method_name):
  return 'NoResampling' if method_name is None else str(method_name)
...
def build_final_decisiontree(best_params_flat):
...
for resample_method in resampling_methods:
  ...
  for fold_idx, (train_idx, test_idx) in enumerate(outer_cv.split(X, y)):
    ...
    best_dt_05 = build_final_decisiontree(best_params_05)
    best_dt_05.fit(X_tr_res_final, y_tr_res_final)

    y_tr_proba_05 = best_dt_05.predict_proba(X_tr_selected)[:, 1]
    y_te_proba_05 = best_dt_05.predict_proba(X_te_selected)[:, 1]
    ...
    best_dt_yj = build_final_decisiontree(best_params_yj)
    best_dt_yj.fit(X_tr_res_final, y_tr_res_final)

    y_tr_proba_yj = best_dt_yj.predict_proba(X_tr_selected)[:, 1]
    y_te_proba_yj = best_dt_yj.predict_proba(X_te_selected)[:, 1]
  ...
  clf_label = f'DecisionTree_{resample_str}_{target_metric.capitalize()}'
...
master_excel_path = os.path.join(
    base_output_dir, 'DecisionTree_Optuna_Master_Summary.xlsx'
)

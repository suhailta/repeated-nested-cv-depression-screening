...
base_seed = 42
base_output_dir = 'results_SVM_RNV'
os.makedirs(base_output_dir, exist_ok=True)

# Outer & Inner Folds Configuration
outer_n_splits = 5
inner_n_splits = 3
...
def precompute_inner_fold_data(
    ...
    for in_tr_idx, in_val_idx in inner_cv.split(X_outer_train, y_outer_train):
        ...
        # 2) Normalization strictly on inner train
        ...
        # Resampling strictly on inner train with k_neighbors=5
...
# Strict Nested Inner Objective - SVM search space
def create_strict_optuna_objective(
    ...
    def objective(trial):
        ...
        gamma_choice = trial.suggest_categorical('gamma_choice', ['float', 'scale'])
...
def build_final_svm(best_params_flat):
    kernel = best_params_flat['kernel']
    degree = best_params_flat['degree'] if kernel == 'poly' else 3

    if best_params_flat['gamma_choice'] == 'float':
        gamma = best_params_flat['gamma_float']
    else:
        gamma = 'scale'

    coef0 = best_params_flat['coef0'] if kernel in ['poly', 'sigmoid'] else 0.0

    return SVC(
        C=best_params_flat['C'],
        kernel=kernel,
        degree=degree,
        gamma=gamma,
        coef0=coef0,
...
for resample_method in resampling_methods:
    folder_resample_name = 'No Resampling' if resample_method == 'No Resampling' else str(resample_method)

    target_dir = os.path.join(
        base_output_dir,
        folder_resample_name,
        'Optimized_Balanced_accuracy',
    )
    dir_m05 = os.path.join(target_dir, 'Threshold_0.5')
    dir_youden = os.path.join(target_dir, 'YoudenJ')
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(outer_cv.split(X, y)):
        ...
        # Resampler with fixed k_neighbors=5
        ...
        # Precompute inner-fold preprocessing ONCE
        ...
        best_svm_05 = build_final_svm(best_params_05)
        best_svm_05.fit(X_tr_res_final, y_tr_res_final)

        y_tr_proba_05 = best_svm_05.predict_proba(X_tr_selected)[:, 1]
        y_te_proba_05 = best_svm_05.predict_proba(X_te_selected)[:, 1]
        ...
        results_05.append({
            'Fold': fold_idx + 1,
            'Resampling_Method': folder_resample_name,
        ...
        best_svm_yj = build_final_svm(best_params_yj)
        best_svm_yj.fit(X_tr_res_final, y_tr_res_final)

        y_tr_proba_yj = best_svm_yj.predict_proba(X_tr_selected)[:, 1]
        y_te_proba_yj = best_svm_yj.predict_proba(X_te_selected)[:, 1]
    ...
    clf_label = f'SVM_{folder_resample_name}_Balanced_accuracy'
...
master_excel_path = os.path.join(
    base_output_dir, 'SVM_Optuna_Master_Summary.xlsx'
)

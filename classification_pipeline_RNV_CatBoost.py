...
base_seed = 42
base_output_dir = 'results_CatBoost_RNV'
...
N_OUTER_REPEATS = 10
N_OPTUNA_TRIALS = 30
SEARCH_N_ITERATIONS = 150
FINAL_N_ITERATIONS = 500
...
def get_resampler(method_name, seed):
  if method_name in [None, 'None', 'NONE', 'No Resampling']:
    ...
    return SMOTETomek(
        random_state=seed, smote=SMOTE(random_state=seed, k_neighbors=5)
    )
  else:
...
# Strict Inner Precomputing & Optuna Objective
def precompute_inner_fold_data(
    X_outer_train, y_outer_train, resample_name, seed, inner_n_splits
):
  ...
  for in_tr_idx, in_val_idx in inner_cv.split(X_outer_train, y_outer_train):
    ...
    # Resampling (fixed k_neighbors=5)
...
def create_strict_optuna_objective(
    precomputed_inner_data, search_n_iterations, threshold_search=False
):
  def objective(trial):
    params = {
        'iterations': search_n_iterations,
...
def build_final_catboost(best_params_flat, n_iterations):
  params = {
      'iterations': n_iterations,
      'learning_rate': best_params_flat['learning_rate'],
      'depth': best_params_flat['depth'],
      'l2_leaf_reg': best_params_flat['l2_leaf_reg'],
      'colsample_bylevel': best_params_flat['colsample_bylevel'],
      'bootstrap_type': best_params_flat['bootstrap_type'],
      'random_seed': 8,
      'eval_metric': 'Logloss',
      'verbose': 0,
      'thread_count': -1,
  ...
  if params['bootstrap_type'] == 'Bernoulli':
    params['subsample'] = best_params_flat['subsample']
  elif params['bootstrap_type'] == 'Bayesian':
    params['bagging_temperature'] = best_params_flat['bagging_temperature']
...
for resample_method in resampling_methods:
  ...
  for fold_idx, (train_idx, test_idx) in enumerate(outer_cv.split(X, y)):
    ...
    best_cat_05 = build_final_catboost(best_params_05, FINAL_N_ITERATIONS)
    best_cat_05.fit(X_tr_res_final, y_tr_res_final)

    y_tr_proba_05 = best_cat_05.predict_proba(X_tr_selected)[:, 1]
    y_te_proba_05 = best_cat_05.predict_proba(X_te_selected)[:, 1]
    ...
    opt_obj_yj = create_strict_optuna_objective(
        precomputed_inner_data, SEARCH_N_ITERATIONS, threshold_search=True
    )
    ...
    best_cat_yj = build_final_catboost(best_params_yj, FINAL_N_ITERATIONS)
    best_cat_yj.fit(X_tr_res_final, y_tr_res_final)

    y_tr_proba_yj = best_cat_yj.predict_proba(X_tr_selected)[:, 1]
    y_te_proba_yj = best_cat_yj.predict_proba(X_te_selected)[:, 1]
  ...
  # Save Output Files
  clf_label = f'CatBoost_{resample_method}_{target_metric.capitalize()}'
...
master_excel_path = os.path.join(
    base_output_dir, 'CatBoost_Optuna_Master_Summary.xlsx'
)

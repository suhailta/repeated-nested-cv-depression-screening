...
from sklearn.linear_model import LogisticRegression as LogRegClassifier
...
base_seed = 42
base_output_dir = 'results_LR_RNV'
...
def resampler_label(method_name):
  return 'No Resampling' if method_name is None else str(method_name)
...
def save_summary_metrics(
  ...
  for col in metric_cols:
    ...
    if n_boot_units >= 2:
      boot_means = [
          np.mean(
              np.random.choice(repeat_values, size=n_boot_units, replace=True)
          )
          for _ in range(1000)
      ]
    ...
    summary_rows.append({
        'Classifier': clf_name,
        'Resampling_Method': str(resample_name),
...
# Precompute Inner-Fold Preprocessing
def precompute_inner_fold_data(
    X_outer_train, y_outer_train, resample_method, seed, inner_n_splits
):
  ...
  for in_tr_idx, in_val_idx in inner_cv.split(X_outer_train, y_outer_train):
    ...
    # Resampling strictly on inner train
    resampler = get_resampler(resample_method, seed)
    ...
    precomputed.append({
        'X_in_tr_res': X_in_tr_res,
        'y_in_tr_res': y_in_tr_res,
        'X_in_val_sel': X_in_val_sel,
        'y_in_val': y_in_val,
    })
...
# Strict Nested Inner Objective (Logistic Regression)
def create_strict_optuna_objective(
    precomputed_inner_data, search_max_iter, threshold_search=False
):
  def objective(trial):
    ...
    if penalty == 'l1':
      ...
      solver = trial.suggest_categorical(
          'solver_none_penalty', ['lbfgs', 'newton-cg', 'sag', 'saga']
      )
      l1_ratio = None

    C = (
        trial.suggest_float('C', 1e-4, 1e4, log=True)
    ...
    params = {
        ...
        'max_iter': (
            search_max_iter if search_max_iter < max_iter else max_iter
        ),
        'random_state': 8,
        'n_jobs': -1,
    }
    ...
    thr = (
        trial.suggest_float('threshold', 0.1, 0.9) if threshold_search else 0.5
    )
    fold_scores = []

    for fold_data in precomputed_inner_data:
      try:
        model = LogRegClassifier(**params)
        model.fit(fold_data['X_in_tr_res'], fold_data['y_in_tr_res'])
        y_val_proba = model.predict_proba(fold_data['X_in_val_sel'])[:, 1]
      except Exception:
        continue

      y_val_pred = (y_val_proba >= thr).astype(int)
      score = balanced_accuracy_score(fold_data['y_in_val'], y_val_pred)
...
def build_final_logreg(best_params_flat, max_iter):
  penalty = best_params_flat['penalty']
  ...
  if penalty == 'l1':
    ...
    solver = 'saga'
    l1_ratio = best_params_flat['l1_ratio']
  elif penalty == 'l2':
    solver = best_params_flat['solver']
    l1_ratio = None
  else:
    solver = best_params_flat['solver_none_penalty']
    l1_ratio = None

  C = best_params_flat.get('C', 1.0)
  ...
  return LogRegClassifier(
      ...
      solver=solver,
      l1_ratio=l1_ratio,
      class_weight=best_params_flat['class_weight'],
      random_state=8,
      max_iter=max_iter,
      n_jobs=-1,
  )
...
# Main Benchmark Loop
resampling_methods = [None, 'SMOTE', 'SMOTETomek']
...
for resample_method in resampling_methods:
  resample_str = resampler_label(resample_method)
  target_metric = 'balanced_accuracy'
  target_dir = os.path.join(
      base_output_dir, resample_str, f'Optimized_{target_metric.capitalize()}'
  )
  dir_m05 = os.path.join(target_dir, 'Default_Threshold_0.5')
  dir_youden = os.path.join(target_dir, 'Method_YoudenJ')
  ...
  for fold_idx, (train_idx, test_idx) in enumerate(outer_cv.split(X, y)):
    ...
    # Precompute inner-fold data
    precomputed_inner_data = precompute_inner_fold_data(
        X_train_outer,
        y_train_outer,
        resample_method,
        current_seed,
    ...
    study_05.optimize(opt_obj_05, n_trials=N_OPTUNA_TRIALS)
    best_params_05 = study_05.best_params.copy()

    best_lr_05 = build_final_logreg(best_params_05, FINAL_MAX_ITER)
    best_lr_05.fit(X_tr_res_final, y_tr_res_final)

    y_tr_proba_05 = best_lr_05.predict_proba(X_tr_selected)[:, 1]
    y_te_proba_05 = best_lr_05.predict_proba(X_te_selected)[:, 1]
    ...
    results_05.append({
        'Fold': fold_idx + 1,
        'Resampling_Method': resample_str,
    ...
    opt_obj_yj = create_strict_optuna_objective(
        precomputed_inner_data, SEARCH_MAX_ITER, threshold_search=True
    )
    study_yj.optimize(opt_obj_yj, n_trials=N_OPTUNA_TRIALS)
    best_params_yj = study_yj.best_params.copy()
    best_threshold = best_params_yj.pop('threshold')

    best_lr_yj = build_final_logreg(best_params_yj, FINAL_MAX_ITER)
    best_lr_yj.fit(X_tr_res_final, y_tr_res_final)

    y_tr_proba_yj = best_lr_yj.predict_proba(X_tr_selected)[:, 1]
    y_te_proba_yj = best_lr_yj.predict_proba(X_te_selected)[:, 1]
  ...
  clf_label = f'LogReg_{resample_str}_{target_metric.capitalize()}'
  ...
  save_confusion_matrix_plot(
      ...
      os.path.join(dir_m05, f'{clf_label}_Default_0.5_confusion_matrix.png'),
  )
...
master_excel_path = os.path.join(
    base_output_dir, 'LogReg_Optuna_Master_Summary.xlsx'
)

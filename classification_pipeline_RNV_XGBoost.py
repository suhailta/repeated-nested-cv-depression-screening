...
base_seed = 42
base_output_dir = 'results_XGBoost_RNV'
...
def get_resampler(method_name, seed):
  if method_name in [None, 'None', 'NONE', 'No Resampling']:
    return None
  elif method_name == 'SMOTE':
    return SMOTE(random_state=seed, k_neighbors=5)
  elif method_name == 'SMOTETomek':
    return SMOTETomek(random_state=seed, smote=SMOTE(random_state=seed, k_neighbors=5))
  else:
...
def get_metrics(y_true, y_pred, y_proba):
  ...
  try:
    ...
    auc = np.nan
  return acc, sens, spec, prec, f1, bal_acc, auc, youden_j, (tn, fp, fn, tp)
...
# Strict Nested Inner Objective (Balanced Accuracy)
def create_strict_optuna_objective(
    precomputed_inner_data,
...
def build_final_xgb(best_params_flat):
  return XGBClassifier(
      learning_rate=best_params_flat['learning_rate'],
      n_estimators=best_params_flat['n_estimators'],
      max_depth=best_params_flat['max_depth'],
      min_child_weight=best_params_flat['min_child_weight'],
      subsample=best_params_flat['subsample'],
      colsample_bytree=best_params_flat['colsample_bytree'],
      gamma=best_params_flat['gamma'],
      reg_alpha=best_params_flat['reg_alpha'],
      reg_lambda=best_params_flat['reg_lambda'],
      random_state=8,
      eval_metric='logloss',
      n_jobs=-1,
      verbosity=0,
...
# Main Loop
resampling_methods = ['No Resampling', 'SMOTE', 'SMOTETomek']
...
for resample_method in resampling_methods:
  target_dir = os.path.join(
      base_output_dir, str(resample_method), 'Optimized_Balanced_accuracy'
  )
  dir_m05 = os.path.join(target_dir, 'Method_Default_Threshold_0.5')
  dir_youden = os.path.join(target_dir, 'Method__YoudenJ')
  ...
  for fold_idx, (train_idx, test_idx) in enumerate(outer_cv.split(X, y)):
    ...
    opt_obj_05 = create_strict_optuna_objective(
        precomputed_inner_data, threshold_search=False
    )
    study_05.optimize(opt_obj_05, n_trials=N_OPTUNA_TRIALS)
    best_params_05 = study_05.best_params.copy()

    best_xgb_05 = build_final_xgb(best_params_05)
    best_xgb_05.fit(X_tr_res_final, y_tr_res_final)

    y_tr_proba_05 = best_xgb_05.predict_proba(X_tr_selected)[:, 1]
    y_te_proba_05 = best_xgb_05.predict_proba(X_te_selected)[:, 1]
    ...
    results_05.append({
        'Fold': fold_idx + 1,
        'Resampling_Method': str(resample_method),
        ...
        **{
        ...
        'Train_Balanced_Accuracy': tr_metrics_05[5],
        'Train_AUC': tr_metrics_05[6],
        'Train_Youden_J': tr_metrics_05[7],
        ...
        'Test_Balanced_Accuracy': te_metrics_05[5],
        'Test_AUC': te_metrics_05[6],
        'Test_Youden_J': te_metrics_05[7],
    })

    # Nested Threshold Tuning
    study_yj = optuna.create_study(direction='maximize')
    opt_obj_yj = create_strict_optuna_objective(
        precomputed_inner_data, threshold_search=True
    )
    ...
    best_xgb_yj = build_final_xgb(best_params_yj)
    best_xgb_yj.fit(X_tr_res_final, y_tr_res_final)

    y_tr_proba_yj = best_xgb_yj.predict_proba(X_tr_selected)[:, 1]
    y_te_proba_yj = best_xgb_yj.predict_proba(X_te_selected)[:, 1]
    ...
    results_youden.append({
        ...
        **{
        ...
        'Train_Balanced_Accuracy': tr_metrics_yj[5],
        'Train_AUC': tr_metrics_yj[6],
        'Train_Youden_J': tr_metrics_yj[7],
        ...
        'Test_Balanced_Accuracy': te_metrics_yj[5],
        'Test_AUC': te_metrics_yj[6],
        'Test_Youden_J': te_metrics_yj[7],
    })

  clf_label = f'XGB_{resample_method}_Balanced_Accuracy'
...
master_excel_path = os.path.join(
    base_output_dir, 'XGB_Optuna_Master_Summary.xlsx'
)
